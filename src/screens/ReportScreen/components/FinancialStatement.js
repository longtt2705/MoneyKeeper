...
const FinancialStatement = ({ navigation }) => {
  ...
  const transactions = useSelector((state) => state.wallets).wallets;
  let sum = 0;
  ...
  function renderFinancialRecent() {
    return (
      ...
      >
        ...
        >
          <Text style={{ ...FONTS.h2, fontWeight: "bold", color: COLORS.blue }}>
            FINANCIAL RECENT
          </Text>
        </View>
        <View
  ...
  return (
    <View style={styles.container}>
      ...
      {listWallet[0] === undefined && <View>{renderEmptyBudget()}</View>}
      {listWallet[0] !== undefined && <View>{renderBudget()}</View>}
    </View>
  );
};
