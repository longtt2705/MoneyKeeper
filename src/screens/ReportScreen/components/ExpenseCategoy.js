...
const ExpenseCategory = () => {
  ...
  for (const id in loadtransactions) {
    ...
    tcategories[indexOfCategory].expenses.push({
      ...
      total: loadtransactions[id].moneyAmount,
      status: confirmStatus,
    });
  }
  let chartCategories = tcategories.filter((e) => e.expenses.length != 0);
  // dummy data
      ...
      >
        ...
        >
          <Text style={{ color: COLORS.white, ...FONTS.body3 }}>
            Price: {formatNumber(item.total)} VND
          </Text>
        </View>
      </View>
    );
    ...
    return (
      <View>
        ...
        {incomingExpenses.length == 0 && (
          ...
          >
            <Text style={{ color: "black", fontWeight: "bold", ...FONTS.h1 }}>
              No Record
            </Text>
          </View>
        )}
    ...
    const renderItem = ({ item }) => (
      ...
      >
        ...
        <View style={{ justifyContent: "center" }}>
          ...
          >
            {formatNumber(item.y)} VND - {item.label}
          </Text>
        </View>
      </TouchableOpacity>
    );
